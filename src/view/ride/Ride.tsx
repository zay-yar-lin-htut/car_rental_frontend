...
const Ride = () => {
	...
	useEffect(() => {
		if (!formValues.pickupDate && !formValues.dropDate) {
			navigate("/");
			return;
		}
	...
	const fetchVehicles = async () => {
		...
		try {
			...
			const data = response.data as { data: SelectedVehicle[]; total: number };
			let fetchedVehicles = data.data || [];
			...
			setTotalPages(Math.ceil(data.total / max));
		} catch (error) {
			showSnackbar(error instanceof Error ? error.message : "Could not fetch vehicles.", "error");
		} finally {
	...
	const handleCarTypeChange = (e: any) => {
  ...
  if (isLoading) {
	...
	const renderCostCalculation = (vehicle: SelectedVehicle) => {
		return (
