...
const CarTypeManagement = () => {
	...
	const [editingCarType, setEditingCarType] = useState<CarTypeRow | null>(null);
	...
	const fetchCarTypes = async () => {
		try {
			...
			const data = (response.data as CarTypeRow[]) || [];
			setCarTypes(data);
			setTotalCarTypes(data.length);
		} catch (err: unknown) {
			const msg = err instanceof Error ? err.message : "Failed to load car types";
			setError(msg);
	...
	const handleOpenDialog = (carType: CarTypeRow | null = null) => {
		...
		setImagePreview(carType ? carType.car_type_image_url || null : null);
	...
	const handleDelete = async (carType: CarTypeRow) => {
