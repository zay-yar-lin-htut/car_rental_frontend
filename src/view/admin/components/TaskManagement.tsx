...
const TaskManagement = () => {
  ...
  const [officeFilter, setOfficeFilter] = useState<string>(""); // Start empty (controlled)
  ...
  const [currentLocation, setCurrentLocation] = useState<LatLngPt | null>(null);
  const [showRoute, setShowRoute] = useState(false);
  const [openConfirm, setOpenConfirm] = useState(false);
  const [selectedTaskForClaim, setSelectedTaskForClaim] = useState<TaskRow | null>(null);
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        const taskList = ((response.data || []) as any[]).map((task: any) => ({
          ...task,
...
useEffect(() => {
    const fetchOfficeLocations = async () => {
      try {
        ...
        const locations = (response.data || []) as OfficeLocation[];
        setOfficeLocations(locations);
        if (!officeFilter && locations.length > 0) {
          setOfficeFilter(String(locations[0].office_location_id));
        }
      } catch (error) {
        ...
        if (!officeFilter) {
          setOfficeFilter(String(fallbackOffices[0].office_location_id));
        }
      }
    };
  ...
  // View Details Dialog
  const handleOpenViewDialog = (task: TaskRow) => {
  ...
  // Claim Task
  const handleClaim = (task: TaskRow) => {
  ...
  const handleOfficeFilterChange = (e: any) => {
    setOfficeFilter(String(e.target.value));
    setPage(0);
  };
...
{officeLocations.length === 0 ? (
      ...
      <ReusableTable
        ...
        keyExtractor={(task: TaskRow) => task.booking_id}
      />
