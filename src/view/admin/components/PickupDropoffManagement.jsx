...
const PickupDropoffManagement = () => {
	...
	const [hasTasks, setHasTasks] = useState(true); // true means no tasks, disable claim
	...
	useEffect(() => {
		const fetchTasks = async () => {
			...
			try {
				...
				setTasks((response.data || []).map(task => ({ ...task, booking_id: task.booking_id || task.id, type: filter === "pickup" ? "Pickup" : "Dropoff" })));
				setError(null);
			} catch {
	...
	const checkHasTasks = useCallback(async () => {
		try {
			...
			setHasTasks(response.data); // data: true if no tasks, false if has tasks
		} catch (error) {
			console.error("Failed to check if staff has tasks:", error);
			setHasTasks(true); // default to no tasks
		}
	}, [dataService]);
	...
	const columns = [
		...
		},
		{
			...
			render: (task) => new Date(task.return_datetime || task.pickup_datetime).toLocaleString(),
		},
		{
			id: "minutes_until",
			label: "Minutes Until",
