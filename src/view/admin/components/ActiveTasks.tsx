...
const ActiveTasks = () => {
	...
	const [selectedTaskForMap, setSelectedTaskForMap] = useState<ActiveTask | null>(null);
	const [currentLocation, setCurrentLocation] = useState<{ lat: number; lng: number } | null>(null);

	// State for Cost Dialog
	const [openCostDialog, setOpenCostDialog] = useState(false);
	const [selectedTaskForComplete, setSelectedTaskForComplete] = useState<ActiveTask | null>(null);
	...
	const columns = [
		...
		},
		{
			...
			render: (task: ActiveTask) => `${task.model} (${task.license_plate})`,
		},
		{
			id: "datetime",
			label: "Date/Time",
	...
	const fetchTasks = useCallback(async () => {
		...
		try {
			...
			setTasks((response.data as ActiveTask[]) || []);
			setError(null);
		} catch {
	...
	const handleOpenMap = (task: ActiveTask) => {
	...
	const handleComplete = async (task: ActiveTask) => {
	...
	const handleNoShow = async (task: ActiveTask) => {
		try {
			await dataService.retrievePOST(
	...
	return (
		...
		<Typography variant="h4" gutterBottom sx={{ color: "var(--text-color)" }}>
			...
			<ReusableTable
				...
				keyExtractor={(task: ActiveTask) => task.task_id}
			/>
