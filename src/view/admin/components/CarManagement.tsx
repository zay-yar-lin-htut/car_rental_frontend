...
const CarManagement = () => {
	...
	const [officeLocations, setOfficeLocations] = useState<LocationRow[]>([]);
	const [carTypes, setCarTypes] = useState<CarTypeRow[]>([]);
	const [editingCar, setEditingCar] = useState<CarRow | null>(null); // State to hold the car being edited
	const [confirmDialog, setConfirmDialog] = useState<{ open: boolean; car: CarRow | null }>({ open: false, car: null });
	...
	useEffect(() => {
		const getOfficeLocation = async () => {
			try {
				...
				setOfficeLocations((response.data as LocationRow[]) || []);
			} catch {
				// office locations are optional; ignore fetch errors
			}
		};
		...
		const getCarTypes = async () => {
			try {
				...
				setCarTypes((response.data as CarTypeRow[]) || []);
			} catch {
				// car types are optional; ignore fetch errors
			}
		};
	...
	const fetchCars = async () => {
		...
		try {
			...
			setTotalCars(raw?.total || raw?.totalCars || 0);
			setError(null);
		} catch (err: unknown) {
	...
	const handleNewCarChange = (event: any) => {
	...
	const validateForm = () => {
		...
		for (const field of requiredFields) {
			if (!carFormData[field] || carFormData[field] === "") {
				newErrors[field] = `${field.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())} is required`;
			}
		}
	...
	const handleEdit = (car: CarRow) => {
		...
		setCarFormData({
			...
			car_type_id: carType ? String(carType.car_type_id) : "",
		});
	...
	const handleConfirmDelete = async () => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : "Failed to delete car.";
	...
	return (
		<Box
			...
			<ConfirmDialog
				...
				title="Delete Car"
				message={`Are you sure you want to delete ${confirmDialog.car?.model}? This action cannot be undone.`}
				confirmText="Delete"
				cancelText="Cancel"
			/>
		</Box>
