...
interface DashboardData {
  ...
  revenue_chart: { labels: (string | number)[]; data: number[] };
  bookings_chart: { labels: (string | number)[]; data: number[] };
  payments_by_staff?: { staff_name: string; total_collected: number }[];
  maintenance_queue?: {
...
const Dashboard = () => {
  ...
  const getStatusChip = (status: string | undefined) => {
    ...
    const key = status || "";
    const config = map[key] || { color: "default", label: key.replace("_", " ").toUpperCase() };
    return (
      <Chip
        label={config.label}
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: "background.default", minHeight: "100vh" }}>
      ...
      {(dashboardData.payments_by_staff?.length ?? 0) > 0 && (
      ...
      {(dashboardData.maintenance_queue?.length ?? 0) > 0 && (
        <Paper sx={{ borderRadius: 3, overflow: "hidden", mb: 5, boxShadow: 3 }}>
          <Box sx={{ p: 3, bgcolor: "primary.main", color: "white" }}>
            <Typography variant="h6" fontWeight="bold">
