import React, { useState, useEffect, useCallback, useMemo, type ChangeEvent, type ReactNode } from "react";
import {
	Box,
	Typography,
	Stack,
...
import { useUserProfile, type UserProfileData } from "./useUserProfile";
...
const ChangePasswordDialog = ({ open, onClose, onSave, isSaving }: ChangePasswordDialogProps) => {
	...
	const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
		...
		setFormData(prev => ({ ...prev, [name]: value } as PasswordFormData));
	};
...
interface EditProfileDialogProps {
	open: boolean;
	onClose: () => void;
	onSave: (data: { formData: Record<string, unknown>; selectedFile: File | null }) => Promise<{ success: boolean; message?: string }>;
	initialData: UserProfileData | null;
	isSaving: boolean;
}

const EditProfileDialog = ({ open, onClose, onSave, initialData, isSaving }: EditProfileDialogProps) => {
	...
	const validateForm = () => {
		...
		if (String(formData.phone || "").trim() && !/^\+?\d{10,15}$/.test(String(formData.phone).replace(/\s/g, "")))
	...
	const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (type === "file") {
			...
			if (previewImage?.startsWith("blob:")) URL.revokeObjectURL(previewImage);
			setPreviewImage(file ? URL.createObjectURL(file) : initialData?.profile_image_url || null);
		} else {
...
const getUserTypeName = (id: number | string | undefined) => {
	const types: Record<number | string, string> = { 1: "User", 2: "Staff", 3: "Admin" };
	return types[id || 0] || "User";
};
							...
							{fineDetails && (
								<Alert severity="error" sx={{ mt: 4, borderRadius: 3, py: 2 }}>
									<Typography fontWeight={700}>
										Outstanding Fine: {fineDetails["Total Fine"] as number} MMK
									</Typography>
								</Alert>
							)}
...
const DetailRow = ({ icon, label, value }: { icon: ReactNode; label: string; value?: string | number }) => (
