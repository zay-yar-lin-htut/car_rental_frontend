...
import type { AuthUser } from "../../contexts/userRoleContext";
...
const Login = () => {
	...
	const [errors, setErrors] = useState<Partial<Record<"email" | "password", string | null>>>({});
	...
	const { updateRole } = useUserRole();
 	...
 	const validateForm = () => {
		const newErrors: Partial<Record<"email" | "password", string | null>> = {};
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			AUTH_CONFIG.setToken(data.token, rememberMe);
			AUTH_CONFIG.setUserData(data.user, rememberMe);

			// Explicitly update the role in the context before navigating
			updateRole(data.user as AuthUser);
