...
import { useIntroForm, type FormLocationValue, type IntroFormValues } from "../../../contexts/IntroFormProvider";
...
let locationsCache: OfficeLocation[] | null = null;
		...
		<Box>
			<Paper
				...
				<Typography
					...
					{(value && value.name) ? (value.name as string) : label}
				</Typography>
			</Paper>
			{error && (
...
const IntroSection = () => {
	...
	const [locations, setLocations] = React.useState<OfficeLocation[]>([]);
	...
	const [locationErrors, setLocationErrors] = React.useState<{ pickup: boolean; dropoff: boolean }>({ pickup: false, dropoff: false });
	const {
		formValues,
		setFormValues,
		expanded,
	...
	useEffect(() => {
		const getOfficeLocation = async () => {
			...
			try {
				...
				locationsCache = (response.data as OfficeLocation[]) ?? null; // Store in cache
				setLocations((response.data as OfficeLocation[]) ?? []);
			} catch {
				// office locations fetch failed; keep previous/empty state
			} finally {
				setIsLoading(false);
			}
		};
	...
	const handleCheckboxChange = (e: ChangeEvent<HTMLInputElement>) => {
		...
		setFormValues({
			...
			dropoffLocation: checked ? formValues.pickupLocation : null,
		});

		if (!expanded) setExpanded(true);
	};

	const handleDateChange = (field: keyof IntroFormValues, value: unknown) => {
	...
	const handleTimeChange = (field: keyof IntroFormValues, value: unknown) => {
		setFormValues({ ...formValues, [field]: value });
	};

	const handleTimeError = (field: string, error: unknown) => {
	...
	const handlePickupSelect = (location: ActiveMarker) => {
		setFormValues((prev) => ({ ...prev, pickupLocation: location as unknown as FormLocationValue }));
		setLocationErrors(prev => ({ ...prev, pickup: false }));
	};

	const handleDropoffSelect = (location: ActiveMarker) => {
		setFormValues((prev) => ({ ...prev, dropoffLocation: location as unknown as FormLocationValue }));
							...
							<>
								...
								<Box sx={{ display: "flex", gap: 2 }}>
									<FutureDatePicker
										...
										maxDate={formValues.dropDate ?? undefined}
								...
								<Box sx={{ display: "flex", gap: 2 }}>
									<FutureDatePicker
										...
										minDate={formValues.pickupDate ?? undefined}
										slotProps={{ textField: { fullWidth: true } }}
									/>
									<TimePicker
										label='Drop Time'
