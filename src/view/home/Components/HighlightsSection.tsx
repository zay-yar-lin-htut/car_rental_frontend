...
const HighlightCard = React.memo(({ item, dataServices }: { item: Highlight; dataServices: ReturnType<typeof createDataServices> }) => {
		...
		<Box sx={{ px: 2, marginTop: 5, marginBottom: 5, outline: "none" }}>
			<Card
				...
				<Box sx={{ position: "relative", height: { xs: 200, md: 250 } }}>
					...
					<CardMedia
						component='img'
						image={dataServices.retrieveImage(item.car_type_image_url as string)}
						alt={item.type_name as string}
...
const HighlightsSection = ({ highlightsData = [], isLoading }: { highlightsData?: Highlight[]; isLoading: boolean }) => {
