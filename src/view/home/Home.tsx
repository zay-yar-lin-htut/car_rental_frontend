...
let highlightsCache: CarType[] | null = null;
...
const CamaroShowcaseHome = () => {
	...
	const [highlightsData, setHighlightsData] = useState<CarType[]>(highlightsCache || []);
	...
	useEffect(() => {
		const fetchHighlightsData = async () => {
			...
			try {
				...
				highlightsCache = response.data as CarType[]; // Store in cache
				setHighlightsData(response.data as CarType[]);
			} catch (error) {
				showSnackbar(error instanceof Error ? error.message : "Failed to load highlights.", "error");
			} finally {
